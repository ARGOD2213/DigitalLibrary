...
- **No API key configured** (`GOOGLE_GENAI_API_KEY` unset/empty): the *hoped-for* behavior is that the application still starts normally and only `POST /api/admin/ai/ask` fails when actually called. But this is **not guaranteed the way the earlier AWS-client comparison implied** — this app's AWS clients are hand-written `@Bean` methods (`AwsConfig.java`) with deliberately lazy credential resolution, a choice this project made; the Google GenAI starter's `@AutoConfiguration` is third-party code whose bean-construction-time behavior (eager validation vs. lazy) is not under this project's control and is unverified as of this spec. The implementer must check this directly (start the app locally with the key unset and see what actually happens) rather than assume it. **If it turns out the autoconfiguration fails context startup without a key**, the fallback is to make the starter's autoconfiguration conditional on the key being present — e.g. wrap the dependency's effect with `@ConditionalOnProperty(name = "...api-key")` at the point this project's own `AiServiceImpl` bean is defined (so the app still boots without a key, and only `AiService`-dependent code fails, with a clear "AI not configured" message, when it's actually used) — rather than requiring every developer to always have a key configured just to run the app locally.
